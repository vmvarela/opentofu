...
- `-refresh=changed` - Enables smart refresh mode, which only refreshes resources
  whose configuration has changed or that have upstream dependencies which changed.
  This can significantly speed up planning for large configurations where only a
  few resources have been modified. Resources not refreshed will use their existing
  state. You cannot use `-refresh=changed` in refresh-only planning mode. Note that
  this option is not supported when using remote execution backends (Terraform Cloud
  or Terraform Enterprise), and a warning will be displayed if used in that context.
