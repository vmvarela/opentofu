...
// or "changed" for the -refresh flag. This enables smart refresh mode where
...
func (r *RefreshFlagValue) Set(s string) error {
	...
	case "changed":
		r.Mode = tofu.RefreshChanged
	default:
		return fmt.Errorf("invalid value %q for -refresh flag; must be true, false, or changed", s)
	}
	return nil
}
...
type Operation struct {
	...
	// Possible values: true (RefreshAll), false (RefreshNone), changed (RefreshChanged).
...
func extendedFlagSet(name string, state *State, operation *Operation, vars *Vars) *flag.FlagSet {
	...
	if operation != nil {
		...
		// Default refresh mode is RefreshAll (equivalent to -refresh=true)
		operation.Refresh.Mode = tofu.RefreshAll
		f.Var(&operation.Refresh, "refresh", "refresh mode: true, false, or changed")
